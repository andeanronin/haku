# Quarterly Report for ETFs in the Bolsa Volares de Lima
...
# Initialize the JSON Search Tool for etf fund data analysis
tool = JSONSearchTool(
    json_path='./etfs-monthly-values.json',
    config={
        "llm": {
            "provider": "openai", 
            "config": {
...
data_analyst = Agent(
    role="Data Analyst",
    goal="Analyze the etf fund data to extract trends, key performance indicators, and insights.",
    backstory="Experienced financial data analyst specializing in etf funds available in Peru.",
    llm="gpt-4o-mini",
    tools=[tool]
)
...
report_writer = Agent(
    role="Report Writer",
    goal="Write a detailed quarterly report summarizing key findings from the etf fund data.",
...
data_analysis_task = Task(
    description="Analyze the etf fund data to identify trends, performance metrics, and key insights for the quarter.",
...
try:
    file_writer_tool._run(filename='etf_quarterly_report.txt', content=str(result), directory='./', overwrite='true')
